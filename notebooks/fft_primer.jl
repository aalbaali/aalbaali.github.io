...
```
where the signal has a frequency of 1 Hz, which is sampled at $f_{s} = 8~\text{Hz}$.
"""

# ╔═╡ c8f47185-4c7c-4c4c-b79d-9c6e4ca77ff1
begin
	N = 8;
	t = (0:N-1) ./ N;
	x = sin.(2π * t);	
...
plot(t, x, markershape=:circle, xlabel="\$t_n\$ [sec]", ylabel="\$x[n]\$")
...
begin
	plot(0:7, abs.(X), xlabel=L"$k = \xi_{k}$ [Hz]", ylabel=L"\vert X(\xi_{k})\vert", label="FFT");
	vline!([N/2], label="Nyquist limit")
end
...
That is,
```math
X_{1/2}[k] = 2X[k], \quad k=0,\ldots,\lceil N/2\rceil
```

And to remedy the second problem, the FFT is normalized with respect the signal length $N$.
That is,
```math
\bar{X}[k] = \frac{1}{N} X[k], \quad k=0,\ldots, N,
```
which is also applicable to the one-sided FFT
```math
\tilde{X}[k]
= \frac{1}{N}X_{1/2}[k]
= \frac{2}{N}X[k]
\quad k=0,\ldots, \lceil N/2 \rceil.
```

The output of after applying these modifications is plotted below.
"""

# ╔═╡ c523902a-dee5-425b-8334-8a518bbef4bc
X_norm = 2 / N * X[1:ceil(Int, N/2)];

# ╔═╡ 388fdaaa-3cc5-4f7b-988d-2ebb96c0bf49
plot(0:N/2-1, abs.(X_norm), xlabel=L"$k = \xi_{k}$ [Hz]", ylabel=L"\vert \tilde{X}(\xi_{k})\vert", marker=:circle)

# ╔═╡ bfa6b7ac-ca32-49fd-8636-40f02e30d989
md"""
# Conclusions
Fourier transforms are at the heart of digital signal processing (DSP) and they are important to understand.
Unfortunately, confusions arise when switching between the different types of Fourier transforms (e.g., FT, DTFT, DFT, etc.) and different types of frequencies (e.g., normalized frequencies, temporal frequencies, etc.).
In this notebook, I tried to demistify *some* of these confusions and used an example to demonstrate how to use the FFTs.
"""

# ╔═╡ 3b3f404f-6d9a-4654-9349-55a2aa4a42b4
md"""
# References
1. A. V. Oppenheim and R. W. Schafer, *Discrete-time signal processing*, 3rd ed. Upper Saddle River: Pearson, 2010.
1. [YouTube video explaining DFT](https://www.youtube.com/watch?v=mkGsMWi_j4Q&t=484s&ab_channel=SimonXu)
1. [Answer about scaling DFT](https://dsp.stackexchange.com/questions/55047/why-do-dft-frequency-buckets-need-to-be-divided-by-sample-period)
1. [3Blue1Brown video](https://www.youtube.com/watch?v=spUNpyF58BY&list=PLH_Vuq97YgbcuEWn1q9iFfucAEynBUYpk&ab_channel=3Blue1Brown)
"""
...
git-tree-sha1 = "1b060125c7a740915da46cb6dcc6817e3bf33c4a"
